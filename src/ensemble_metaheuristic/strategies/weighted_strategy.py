"""Classic weighted ensemble strategy: random search + hill climb over weights and thresholds."""
...
def _run_standalone_cli() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Classic weighted ensemble search only (random + hill climb).",
    )
    parser.add_argument("--config", default="src/evaluation/config.yaml", help="Evaluation YAML (config.yaml).")
    ...
    print("Classic weighted search (this module only)")
    ...
    for r in range(max(1, int(args.restarts))):
        ...
        w, mt, gt, f1 = run_search(
            matrices, is_score_model, gt_data, all_pids, all_labels, int(args.n_iter), rng, verbose=(r == 0),
        )
