...
Όλος ο κώδικας pipeline για το XLM-R βρίσκεται στο πακέτο `src/xlm_r` (underscore — απαιτείται από την Python· δεν μπορεί να υπάρχει `xlm-r` ως όνομα module).
...
2. **Threshold Tuning (Βελτιστοποίηση Κατωφλίων):**
   ...
   python -m src.evaluation.threshold_tune \
       --scores outputs/xlm_r/val_scores.npy \
       --pids outputs/xlm_r/val_patient_ids.json \
       --labels outputs/xlm_r/label_names.json \
       --ground-truth data/processed/validation_set.jsonl \
       --out outputs/xlm_r/thresholds.json
   ```
...
4. **Αξιολόγηση (Validation):**
   ...
   python -m src.evaluation.evaluator \
       --ground-truth data/processed/validation_set.jsonl \
       --pred outputs/xlm_r/val_predictions.jsonl
   ```
...
- `src/xlm_r/train.py`: Εκπαίδευση (AMP, warmup scheduler, validation).
