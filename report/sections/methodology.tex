...
CLS extraction by $\sim$0.8 F1 points~\cite{grbert_doc_pooling}, as diagnostic evidence
...
BCE baseline (0.74) $\to$ ASL (0.788) $\to$ LLRD + MLP (0.811) $\to$ final P4 tuning
(0.7984 base at $t{=}0.6$, 0.8062 after per-label threshold tuning).
...
A two-pass sweep (step 0.01) selects $t_l^{*} = \arg\max_{t \in [0.45,0.75]} F1_l(t;\mathcal{D}_{\text{val}})$ per label independently. Pass~2 accepts per-label values only for labels with $\geq$20 positive examples and $\Delta$F1 $>0.0015$. This adds $\approx$0.8 F1 points over the active threshold ($t{=}0.6$) and $\approx$3.7 points over a fixed $t{=}0.5$ baseline, bringing the tuned validation micro-F1 to 0.8062.
...
(0.8101 vs.\ 0.7538) due to the semantic anchoring mechanism and task-specific
