...
Figure~\ref{fig:compbar} shows standalone validation micro-F1 for each component.
Greek BERT is the strongest single model. Dictionary and NER+EL perform comparably;
the IR module has the lowest F1 but the highest recall (0.83), making it a key
coverage contributor in the ensemble.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/fig_validation_micro_f1_by_component_bar.pdf}
\caption{Standalone validation micro-F1 for each system component.}
\label{fig:compbar}
\end{figure}
...
Layer-wise learning rate decay improved validation F1 by $\approx$0.4 points, most
notably for rare labels where preserving pre-trained lower-layer representations
matters most.
...
The ensemble outperforms the best standalone model by +1.8 F1 points (0.8667 vs.\ 0.8489),
...
\caption{Macro-F1 per component across label frequency bands. All models degrade sharply on rare labels ($<$10 instances); the ensemble provides no recovery for this band.}
\label{fig:freqband}
\end{figure}
...
\begin{minipage}{0.55\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/fig_top_fp_fn_labels.pdf}
  \caption{Top false-positive and false-negative codes for the best ensemble submission.}
  \label{fig:fpfn}
\end{minipage}\hfill
\begin{minipage}{0.42\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/fig_acute_mi_codes_confusion_cluster.pdf}
  \caption{Co-prediction counts for the MI/procedure cluster (I21, I22, I25, Z95, Y84).}
  \label{fig:micluster}
\end{minipage}
\end{figure}

Rare labels. The 30 codes with $<$10 training instances are the primary error source.
...
Coverage gaps. 31 label types lie outside the dictionary's coverage, including metabolic
...
Label confusion. I21/I22/I25 (acute MI, subsequent MI, chronic ischaemic disease) frequently
...
Calibration. Greek BERT under-predicts rare labels: probabilities for rare codes often fall
