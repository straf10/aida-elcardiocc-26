...
Figure~\ref{fig:compbar} shows standalone micro-F1 on our test set for each component.
Greek BERT is the strongest single model (0.8196), ahead of XLM-RoBERTa Base (0.8050).
...
\caption{Standalone micro-F1 on our test set for each system component.}
\label{fig:compbar}
\end{figure}
...
Per-label tuning adds $\approx$3.7 F1 points over a fixed $t{=}0.5$ baseline and
$\approx$0.7 points over the global optimum.
